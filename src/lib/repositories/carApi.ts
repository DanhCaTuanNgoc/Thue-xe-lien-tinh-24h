...
export async function checkCarExists(
   province: string,
   end_location: string,
   id_car_type: number,
): Promise<boolean> {
...
export async function addCar(car: Omit<Car, 'id'>) {
   ...
   if (exists) {
      throw new Error(
         `${carType?.name} đến ${car.province}, ${car.end_location} đã tồn tại trong hệ thống!`,
      )
   }
   ...
   if (car.province || car.end_location || car.id_car_type) {
      ...
      if (currentCar) {
         ...
         if (existingCar) {
            ...
            throw new Error(
               `${carType?.name} đến ${newProvince}, ${newEndLocation} đã tồn tại trong hệ thống!`,
            )
         }
      }
   }
